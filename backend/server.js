...
app.use(
  cors({
    origin: "*", // cho phép mọi origin gọi (FE + Git Bash)
  })
);
...
function verifySignature(req, res, next) {
  ...
  const isTrustedOrigin =
    ...
    origin.startsWith("https://tumainguyen.github.io");

  // Nếu request đến từ FE của mình (localhost hoặc GitHub Pages)
  // và KHÔNG có x-signature -> cho qua, không check HMAC
  if (isTrustedOrigin && !req.header("x-signature")) {
    console.log("[webhook] skip HMAC for trusted origin:", origin);
    return next();
  }

  // Còn lại (Git Bash, tool bên ngoài...) thì bắt buộc phải có HMAC
  if (!WEBHOOK_SECRET) {
...
app.get("/admin/events", (req, res) => {
  res.json({
    ok: true,
    items: [...events].reverse(), // event mới lên đầu
  });
});
